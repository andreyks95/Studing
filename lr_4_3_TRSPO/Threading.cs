        ...
        private static void Main(string[] args)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    var taskGetMax = GetMax(arrayValues);
                    taskGetMax.Wait();
                    taskGetMax.Dispose();
                    Console.WriteLine("Наибольшее значение функции: " + taskGetMax.Result);
                    sw.Stop();
        ...
        Task.Run(() =>
        {
            ...
            System.Threading.Tasks.Parallel.ForEach(
               ...
               (index, state, localList) =>
               {
                   ...
                   var task = GetMax(GetValuesY(startY, endY, stepY, index));
                   task.Wait();
                   task.Dispose();
                   localList.Add(task.Result);
                   if (localList.Count > 1000)
                   {
                       task = GetMax(localList);
                       task.Wait();
                       task.Dispose();
                       localList.Clear();
